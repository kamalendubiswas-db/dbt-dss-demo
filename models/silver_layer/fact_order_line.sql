...
FROM
  dim_order ord
  LEFT JOIN   lineitems     lnt   ON  lnt.order_id      = ord.order_id
  LEFT JOIN   dim_supplier  sup   ON  sup.supplier_id   = lnt.supplier_id
  LEFT JOIN   dim_part      part  ON  part.part_id      = lnt.part_id
  LEFT JOIN   dim_customer  cust  ON  cust.customer_id  = ord.customer_id
  LEFT JOIN   dim_nation    nat   ON  sup.nation_id     = nat.nation_id